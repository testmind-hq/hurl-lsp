...
Resolved values are previewed as inline hints. Sensitive names such as tokens, passwords, and authorization values are masked. Configure this with `hurl.variables.inlayHints.enabled` and `hurl.variables.inlayHints.maxLength`.
...
- `⚡ Run with vars` action (merges effective `.hurl-vars` / `vars.env` / `hurl.env` / `.env` values using workspace precedence)
...
- `📋 Copy as curl` action (resolves workspace variables, copies directly to the clipboard, and opens the cURL Inspector view)
...
Command `Hurl: Open Inspector` opens a side panel with:

- `Request`: request details and Run / Run with vars / Run Chain / Copy as cURL actions
- `Chain`: entry list plus inferred and explicit dependency edges
- `Result`: structured request and response headers/body, JSON formatting, assertions, raw output, and the latest 10 in-memory runs
- `cURL`: complete generated command, masked secret preview, reveal/hide, and copy again

`Copy as cURL` is static and never sends a network request. It supports URL, headers, query, Basic Auth, JSON/XML/text bodies, forms, and multipart requests. Runtime captures and built-in dynamic values must be available as static variables before copying.
...
Install the **Hurl LSP** extension from the [VS Marketplace](https://marketplace.visualstudio.com) or [Open VSX](https://open-vsx.org/extension/testmind-hq/vscode-hurl) (VSCodium). Search for `hurl-lsp` / `Hurl`.  
Listing copy lives in [`editors/vscode/README.md`](editors/vscode/README.md).
